...
@pytest.mark.skip(reason="only use this on cluster_stdv2_1")
@pytest.mark.parametrize('region', ['REGION_EU'])
def test_cluster_stdv2_1(session_ctxt: SessionContext, device_factory: DeviceFactory, region: ZwaveRegion):
    ...
    end_device_list = []
    for i in range(1, 10):
        zpc.logger.info(f"spawning end_device {i} ...")
        end_device_list.append(device_factory.switch_on_off(region))

    for end_device in end_device_list:
        zpc.add_node()
        end_device.set_learn_mode()
        zpc.wait_for_node_connection(end_device)

    # and that's it, if we can include all devices then the cluster should be working.


@pytest.mark.skip(reason="only use this on cluster_stdv2_1")
@pytest.mark.parametrize('region', ['REGION_EU'])
def test_cluster_stdv2_1_railtest(device_factory: DeviceFactory, region: ZwaveRegion):
    _logger = logging.getLogger(__name__)
