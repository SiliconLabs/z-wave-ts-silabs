...
class ZpcTestSystem(BackgroundProcess):

    def __init__(self, controller_name: str, ctxt: SessionContext, region: str, tty_path: str, mqtt_sock_path: Path):
        self.mqtt_main_process: Mosquitto | None = None
        self.mqtt_logs_process: MosquittoSub | None = None
        self.tty_path: str = tty_path
        self.mqtt_host: Path = mqtt_sock_path

        self.zpc_instance_name = f"zpc-{controller_name}"

        uic_config_file_path = f"{ctxt.current_test_logdir}/uic-{self.zpc_instance_name}.cfg"
        with open(uic_config_file_path, "w") as uic_cfg:
            uic_cfg.write(self._generate_uic_configuration_file(
                zpc_instance_name=self.zpc_instance_name,
                ctxt=ctxt,
                region=region.replace('REGION_', ''),
                log_level='d',
                tx_power='0',
        ...
        cmd_line = f'{ctxt.uic}/build/applications/zpc/applications/zpc_test_system/zpc_test_system --conf {uic_config_file_path}'
        self._start_mqtt_processes(ctxt, mqtt_sock_path)
        self.patterns = {
            r"We are connected to the MQTT broker.": None,
            r"Subscription to ucl\/by-unid\/zw-(([A-F]|[0-9]){8})-(\d{4})\/ProtocolController\/NetworkManagement\/Write successful": None
        }
        super().__init__(ctxt, self.zpc_instance_name, cmd_line, self.patterns, 30)


    def _generate_uic_configuration_file(self, zpc_instance_name: str, ctxt: SessionContext, region: str, log_level: str, tx_power: str, protocol_pref: str) -> str:
        uic_configuration = (
            f"# Unify configuration file (autogenerated on: {datetime.now()})\n"
            f"log:\n"
            f"  level: '{log_level}'\n"
            ...
            f"  rf_region: '{region}'\n"
            f"  serial: '{self.tty_path}'\n"
            f"  serial_log_file: '{ctxt.current_test_logdir}/sapi-{zpc_instance_name}.log'\n"
            f"  datastore_file: '{ctxt.current_test_logdir}/{zpc_instance_name}.db'\n"
        )
