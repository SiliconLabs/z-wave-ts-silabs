...
Examples:
  ...
  %(prog)s --branch feature/my-feat # Use 'feature/my-feat' branch artifacts
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Erase dist/bin first, then always download Conan metadata and overwrite (ignore existing manifest)"
    )
    ...
    # --force: start by erasing dist/bin
    if args.force:
        log("Force: erasing dist/bin...", "info")
        if not clean_dist_bin(dist_bin):
            sys.exit(1)
    
    # Download metadata (skip if manifest already present, unless --force)
    if not args.force and manifest_file.exists():
        log("Skipping Conan metadata download (manifest already present)", "warn")
    elif not download_conan_metadata(package_ref, remote_name):
        sys.exit(1)
