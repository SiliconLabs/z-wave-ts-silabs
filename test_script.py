...
def test_switch_on_off_secure_and_unsecure_inclusion_exclusion(get_wpks_from_cluster):
    wpks = get_wpks_from_cluster('stdv1-1')
    zpc = DevZwaveGwZpc('zpc', wpks[-1], 'REGION_EU')
    end_device_1 = DevZwaveSwitchOnOff('end_device_1', wpks[0], 'REGION_EU')
    end_device_2 = DevZwaveSwitchOnOff('end_device_2', wpks[1], 'REGION_EU')
    ...
    zpc.add_node()
    end_device_2.set_learn_mode()
    zpc.wait_for_node_connection(end_device_2)

    end_device_1.toggle_led()
    # check that we've received a MQTT update on this

    end_device_2.toggle_led()
    # check that we've received a MQTT update on this

    end_device_1.send_nif()
    # check that we've received a MQTT update on this

    end_device_2.send_nif()
    # check that we've received a MQTT update on this

    # TODO:
    # Switch Off LED of end_device1 from mqtt
    # check that we've received a MQTT update on this

    # TODO:
    # Switch Off LED of end_device2 from mqtt
    # check that we've received a MQTT update on this

    # exclusion of end_device1
    zpc.remove_node()
    end_device_1.set_learn_mode()
    zpc.wait_for_node_disconnection(end_device_1)

    # exclusion of end_device2
    zpc.remove_node()
    end_device_2.set_learn_mode()
    zpc.wait_for_node_disconnection(end_device_2)

    # secure inclusion of end_device1
    zpc.add_node(end_device_1.get_dsk())
    end_device_1.set_learn_mode()
    zpc.wait_for_node_connection(end_device_1)

    # secure inclusion of end_device2
    zpc.add_node(end_device_2.get_dsk())
    end_device_2.set_learn_mode()
    zpc.wait_for_node_connection(end_device_2)
